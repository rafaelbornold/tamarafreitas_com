import KEY from "./stripe_js_keys.json" assert { type: "json" };
...
async function checkStatus(stripe) {
  ...
  switch (paymentIntent.status) {
    ...
    case "processing":
      showMessage("TU PAGO SE ESTÁ PROCESANDO. ENTRE EN CONTACTO CON EL STUDIO PARA CONFIRMAR EL PAGO", "status_warning");
      break;
        ...
        getInfoPlazaPeriod(formDatas).then( infoPlazaPeriod => { 
          ...
          if (infoPlazaPeriod.plazas >= infoPlazaPeriod.countSelectedProcedures) {
            ...
            if (! activePayment){
              ...
              alert("Se han agotado las plazas por la cantidad de procedimientos que desea realizar. Seleccione un número menor de procedimientos o intente el proceso nuevamente.");
            }    
        });
            ...
            if (result.paymentIntent) {
              ...
              confirmPaymentOnDB(result, registers).then( confirmation => { 
                ...
                if (confirmation.DB_registerUpdated === true) {
                  window.location.href = '/reservas/index.php?checkout=Finished&nif_session='+formDatas[0]['nif']+'&payment_intent='+result.paymentIntent.id+'&payment_intent_client_secret='+result.paymentIntent.client_secret;
                } else {
                  var message = "Erro no momento de registrar o pagamento no Bando de Dados. Se você receber o Recibo por e-mail, por favor entre em contato com a recepção da Tamara Freitas para completar seu registro. | Menssagem de erro: " + confirmation.DB_registerUpdated;
                  showMessage(message, "status_success");
                }
